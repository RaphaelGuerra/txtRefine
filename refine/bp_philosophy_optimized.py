...
class OptimizedBPPhilosophySystem:
    ...
    def _build_efficient_corrections(self):
        """Build efficient unified corrections dictionary."""
        # Unified corrections dictionary for maximum efficiency
        self.corrections = {}
        self._canonical_names = set()
    ...
    def _is_name_like(self, token: str) -> bool:
        """Heuristic: looks like a proper name token."""
    ...
    def get_brazilian_context_info(self, term: str) -> Dict:
        """Get Brazilian Portuguese specific context information."""
        term_lower = term.lower()

        for category, patterns in [('Phonetic variation', {}), ('Academic expression', {})]:
    ...
    def validate_brazilian_accentuation(self, text: str) -> List[str]:
        """Validate Brazilian Portuguese accentuation."""
        suspicious_terms = []
        words = re.findall(r'\b[a-zA-ZÀ-ÿ]+\b', text)

        accentuation_patterns = {
            'ontologia': 'ontologia', 'metafísica': 'metafísica', 'filosofia': 'filosofia',
            'conhecimento': 'conhecimento', 'argumentação': 'argumentação',
            'interpretação': 'interpretação', 'compreensão': 'compreensão',
            'existência': 'existência', 'essência': 'essência', 'substância': 'substância',
            'consciência': 'consciência', 'transcendência': 'transcendência',
            'imanência': 'imanência'
        }
    ...
    def get_whisper_optimization_tips(self) -> List[str]:
        """Get tips for optimizing Whisper LLM performance."""
        return [
